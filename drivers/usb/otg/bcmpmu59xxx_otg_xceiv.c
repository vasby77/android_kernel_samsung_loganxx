...
static int bcmpmu_otg_xceiv_start(struct usb_phy *phy)
{
	...
	if (xceiv_data) {
		printk("USBD] otg_enable=%x, regulator eanble =%x ,xceiv_data->bcm_hsotg_regulator = %x\n",xceiv_data->otg_enabled,xceiv_data->regulator_enabled,xceiv_data->bcm_hsotg_regulator);
		...
		if (!xceiv_data->otg_enabled) {
			if (xceiv_data->bcm_hsotg_regulator &&
				...
				printk("%s-59xxx-regulator_enable(xceiv_data->bcm_hsotg_regulator)\n",__func__);
				ret = regulator_enable(xceiv_data->bcm_hsotg_regulator);
				if(!ret){
				/* Give 2ms to ramp up USBLDO */
				mdelay(USBLDO_RAMP_UP_DELAY_IN_MS);
				xceiv_data->regulator_enabled = true;
					//add code to check whether the usb ldo is really turned on or not.
					printk("%s-USBLDO ON:%d\n",__func__,regulator_is_enabled(xceiv_data->bcm_hsotg_regulator));
				}
				else{
					printk("%s-Fail to regulator_enable\n",__func__);
					return -ENODEV;
				}
		...
		ret = bcm_hsotgctrl_phy_init(!id_default_host);
		if(ret){
			printk("%s-Fail to USB Phy Init\n",__func__);
			return -ENODEV;
		}
					   ...
					   unsigned int ma)
{
	...
	printk("USBD][bcmpmu_otg_xceiv_set_vbus_power] just return 0\n");  
	return 0;			   
}
...
static int __devinit bcmpmu_otg_xceiv_probe(struct platform_device *pdev)
{
	...
	usb_phy_init(&xceiv_data->otg_xceiver.phy);
