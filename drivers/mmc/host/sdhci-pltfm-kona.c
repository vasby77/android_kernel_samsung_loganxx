...
kona_sdio_regulator_power(struct sdio_dev *dev, int power_state)
{
	...
	if (dev->devtype == SDIO_DEV_TYPE_SDMMC && dev->vddo_sd_regulator &&
		...
		if (power_state){
			if(!dev->sdio_regulator_enable) {
				dev_info(dev->dev, "Turning ON sdldo and sdxldo\r\n");
				...
				printk("%s-dev->sdio_regulator_enable:%d-->Not regulator_enable\n"
					,__func__,dev->sdio_regulator_enable);
			}
		} 
		else{
			if(dev->sdio_regulator_enable) {
				dev_info(dev->dev, "Turning OFF sdldo and sdxldo\r\n");
				...
				printk("%s-dev->sdio_regulator_enable:%d-->Not regulator_disable\n"
					,__func__,dev->sdio_regulator_enable);
			}
		}
	}
