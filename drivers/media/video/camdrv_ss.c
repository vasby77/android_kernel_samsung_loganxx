...
static int camdrv_ss_set_effect(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	state->currentEffect=ctrl->value;
	return err;
}
...
static int camdrv_ss_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (ctrl->id == V4L2_CID_CAMERA_INITIALIZE) {
		...
		if ((ctrl->value == 1) && (sensor.smartStayChangeInitSetting != NULL)) {
			CAM_INFO_PRINTK("%s: smartStayChangeInitSetting called ..\n", __func__);
			sensor.smartStayChangeInitSetting(&sensor);
		}
		...
		if (atomic_read(&sensor_state) == CAMDRV_SS_NOT_INITIALIZED) {
			...
			/* Sensor power up is sequential function , After it is successful create seperate thread for init */
			if( 0 != camdrv_ss_actual_sensor_power_up() )
			{
				CAM_ERROR_PRINTK("%s %s :camdrv_ss_actual_sensor_power_up FAILED. Return ERR !!!!.\n", sensor.name, __func__);
				return -1;
			}
	...
	{
		if (sensor.flash_control != NULL)
			err = sensor.flash_control(sd, ctrl->value);
		break;
	}
	...
	case V4L2_CID_CAMERA_WHITE_BALANCE:
	{
		...
		if (sensor.set_white_balance != NULL) {
			...
			err = sensor.set_white_balance(sd, ctrl->value);
	...
	case V4L2_CID_CAMERA_ISO:
	{
		...
		err = sensor.set_iso(sd, ctrl->value);
		break;
	}
	...
	if (bOn) {
		...
		if (camera_id == CAMDRV_SS_CAM_ID_MAIN) {
			...
			if ( sensor.rear_camera_vendorid != NULL){
				...
				CAM_ERROR_PRINTK(
						"%s %s : camdrv_rear_camera_vendorid\n",
