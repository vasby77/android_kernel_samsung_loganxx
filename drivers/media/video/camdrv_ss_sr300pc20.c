...
/***********************************************************************
* Driver for sr300pc20 (5MP Camera) from SAMSUNG SYSTEM LSI
*
...
#define SR300PC20_DEFAULT_PIX_FMT		V4L2_PIX_FMT_UYVY	/* YUV422 */
#define SR300PC20_DEFAULT_MBUS_PIX_FMT    V4L2_MBUS_FMT_UYVY8_2X8 
#define FORMAT_FLAGS_COMPRESSED 0x3
#define DEFUALT_MCLK		26000000

static bool first_af_status = false;
#define SR300PC20_REGISTER_SIZE 2

#define SR300PC20_DELAY_DURATION 0xFF
...
static const struct camdrv_ss_framesize  sr300pc20_supported_capture_framesize_list[] = {
...
const static struct v4l2_fmtdesc sr300pc20_fmts[] = 
{
    {
        .index          = 0,
...
static const struct v4l2_queryctrl sr300pc20_controls[] = {
	{
...
static int camdrv_ss_sr300pc20_enum_frameintervals(struct v4l2_subdev *sd,struct v4l2_frmivalenum *fival)
{
	int err = 0;
	int size,i;
	...
	for(i = 0; i < ARRAY_SIZE(sr300pc20_supported_preview_framesize_list); i++) {
		if((sr300pc20_supported_preview_framesize_list[i].width == fival->width) &&
		    (sr300pc20_supported_preview_framesize_list[i].height == fival->height))
		{
			size = sr300pc20_supported_preview_framesize_list[i].index;
			break;
		}
	}

	if(i == ARRAY_SIZE(sr300pc20_supported_preview_framesize_list))
	{
...
static long camdrv_ss_sr300pc20_ss_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
...
int camdrv_ss_sr300pc20_set_preview_start(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
                                         ...
                                         regs_t reg_buffer[],int num_of_regs,char *name)
{
    ...
    for(i = 0; i < num_of_regs; i++)
    {
        ...
        switch(subaddr)
        {
            ...
            default:
            {
				if(SR300PC20_REGISTER_SIZE == 4)
				      err = camdrv_ss_i2c_write_4_bytes(client, subaddr, data_value);
				else if(SR300PC20_REGISTER_SIZE == 2)
				{
...
static float camdrv_ss_sr300pc20_get_exposureTime(struct v4l2_subdev *sd)
{
#if 0
    struct i2c_client *client = v4l2_get_subdevdata(sd);	
...
//aska add
static int camdrv_ss_sr300pc20_get_nightmode(struct v4l2_subdev *sd)
{
	return 0;
};

static int camdrv_ss_sr300pc20_get_iso_speed_rate(struct v4l2_subdev *sd)
{
#if 0	
    struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int camdrv_ss_sr300pc20_get_ae_stable_status(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{   
#if 0
    struct i2c_client *client = v4l2_get_subdevdata(sd);
    int err = 0;
...
#if 0
static int camdrv_ss_sr300pc20_get_auto_focus_status(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{ 
    struct i2c_client *client = v4l2_get_subdevdata(sd);
    ...
    printk("camdrv_ss_sr300pc20_get_auto_focus_status E \n");
    do
    {
        err = camdrv_ss_i2c_set_config_register(client,sr300pc20_get_1st_af_search_status,ARRAY_SIZE(sr300pc20_get_1st_af_search_status),"get_1st_af_search_status");
    ...
    do
    {
        ...
        err = camdrv_ss_i2c_set_config_register(client,sr300pc20_get_2nd_af_search_status,ARRAY_SIZE(sr300pc20_get_2nd_af_search_status),"get_2nd_af_search_status");
        err += camdrv_ss_i2c_read_2_bytes(client, 0x0F12, &usReadData);
        if(err < 0)
        {
            CAM_ERROR_PRINTK( "%s: I2C failed during AF \n", __func__);
...
static int camdrv_ss_sr300pc20_get_touch_focus_status(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
      printk("camdrv_ss_sr300pc20_get_touch_focus_status E \n");
 	return 0;
}


static int camdrv_ss_sr300pc20_set_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    struct i2c_client *client = v4l2_get_subdevdata(sd);
	  struct camdrv_ss_state *state = to_state(sd);
    int err = 0;
	...
	printk("camdrv_ss_sr300pc20_set_auto_focus E \n");
    ...
    if(ctrl->value == AUTO_FOCUS_ON) 
    {
		...
		if(sr300pc20_single_af_start_regs == 0)
			CAM_ERROR_PRINTK( "%s : single_af_start_regs  supported !!! \n", __func__);
                ...
                {      
                        ...
                        err = camdrv_ss_i2c_set_config_register(client,sr300pc20_ae_lock_regs, ARRAY_SIZE(sr300pc20_ae_lock_regs), "ae_lock_regs");

                        CAM_INFO_PRINTK( "%s , AF start\n ",__func__);

			err = camdrv_ss_i2c_set_config_register(client,sr300pc20_single_af_start_regs, ARRAY_SIZE(sr300pc20_single_af_start_regs),"single_af_start_regs");
                }
    ...
    else if(ctrl->value == AUTO_FOCUS_OFF) 
    {
        ...
        state->camera_flash_fire = 0;
        state->camera_af_flash_checked = 0;
		if(sr300pc20_single_af_stop_regs == 0)
			CAM_ERROR_PRINTK( "%s : single_af_stop_regs  supported !!! \n", __func__);
		else
			err = camdrv_ss_i2c_set_config_register(client,sr300pc20_single_af_stop_regs,ARRAY_SIZE(sr300pc20_single_af_stop_regs),"single_af_stop_regs");
		 if(err < 0)
    	 {
...
static int camdrv_ss_sr300pc20_set_touch_focus(struct v4l2_subdev *sd, enum v4l2_touch_af touch_af, v4l2_touch_area *touch_area)
{
	struct camdrv_ss_state *state = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
#if 0
static int camdrv_ss_sr300pc20_get_light_condition(struct v4l2_subdev *sd, int *Result)
{
    struct i2c_client *client = v4l2_get_subdevdata(sd);	
...
static bool camdrv_ss_sr300pc20_check_flash_needed(struct v4l2_subdev *sd)
{
#if 0
    struct i2c_client *client = v4l2_get_subdevdata(sd);	
...
#if 0
static int camdrv_ss_sr300pc20_AAT_flash_control(struct v4l2_subdev *sd, int control_mode)
{
    switch(control_mode)
    {
        // USE FLASH MODE
...
bool camdrv_ss_sensor_init_main(bool bOn, struct camdrv_ss_sensor_cap *sensor)
{
	...
	sensor->supported_preview_framesize_list  = sr300pc20_supported_preview_framesize_list;
	sensor->supported_number_of_preview_sizes = ARRAY_SIZE(sr300pc20_supported_preview_framesize_list);
	...
	sensor->controls				   =sr300pc20_controls;
	sensor->rows_num_controls	      =ARRAY_SIZE(sr300pc20_controls);
	
	sensor->default_pix_fmt 				   = SR300PC20_DEFAULT_PIX_FMT;
	sensor->default_mbus_pix_fmt			   = SR300PC20_DEFAULT_MBUS_PIX_FMT;
	sensor->register_size 		  			 = SR300PC20_REGISTER_SIZE;

	sensor->delay_duration				= SR300PC20_DELAY_DURATION;

	/* sensor dependent functions */
	
/* mandatory*/
	sensor->thumbnail_ioctl			       = camdrv_ss_sr300pc20_ss_ioctl;
	sensor->enum_frameintervals			   = camdrv_ss_sr300pc20_enum_frameintervals;
...
//	sensor->get_light_condition   = camdrv_ss_sr300pc20_get_light_condition;
	...
	sensor->sensor_power			= camdrv_ss_sr300pc20_sensor_power;		  

	sensor->get_exif_sensor_info =
		camdrv_ss_sr300pc20_get_sensor_param_for_exif;
	

	/*REGS and their sizes*/
	/* List all the capabilities of sensor . List all the supported register setting tables */
	
	sensor->init_regs						  = sr300pc20_init_regs;
	sensor->rows_num_init_regs				  = ARRAY_SIZE(sr300pc20_init_regs);
	
	sensor->preview_camera_regs 			  = sr300pc20_preview_camera_regs;
	sensor->rows_num_preview_camera_regs 	  = ARRAY_SIZE(sr300pc20_preview_camera_regs);
			
	/*snapshot mode*/
	sensor->snapshot_normal_regs			  =	sr300pc20_snapshot_normal_regs;
	sensor->rows_num_snapshot_normal_regs	  = ARRAY_SIZE(sr300pc20_snapshot_normal_regs);

	sensor->snapshot_lowlight_regs			  =	sr300pc20_snapshot_lowlight_regs;
	sensor->rows_num_snapshot_lowlight_regs	  = ARRAY_SIZE(sr300pc20_snapshot_lowlight_regs);

	sensor->snapshot_highlight_regs			  =	sr300pc20_snapshot_highlight_regs;
	sensor->rows_num_snapshot_highlight_regs	  = ARRAY_SIZE(sr300pc20_snapshot_highlight_regs);

	sensor->snapshot_nightmode_regs			  =	sr300pc20_snapshot_nightmode_regs;
	sensor->rows_num_snapshot_nightmode_regs	  = ARRAY_SIZE(sr300pc20_snapshot_nightmode_regs);

	sensor->snapshot_flash_on_regs			  =	sr300pc20_snapshot_flash_on_regs;
	sensor->rows_num_snapshot_flash_on_regs	  = ARRAY_SIZE(sr300pc20_snapshot_flash_on_regs);

	sensor->snapshot_af_preflash_on_regs			  =	sr300pc20_snapshot_af_preflash_on_regs;
	sensor->rows_num_snapshot_af_preflash_on_regs	  = ARRAY_SIZE(sr300pc20_snapshot_af_preflash_on_regs);

	sensor->snapshot_af_preflash_off_regs			  =	sr300pc20_snapshot_af_preflash_off_regs;
	sensor->rows_num_snapshot_af_preflash_off_regs	  = ARRAY_SIZE(sr300pc20_snapshot_af_preflash_off_regs);

#if 0
	sensor->focus_mode_macro_regs			  =	sr300pc20_af_macro_mode_regs;
	sensor->rows_num_focus_mode_macro_regs	  = ARRAY_SIZE(sr300pc20_af_macro_mode_regs);
	...
	sensor->effect_negative_regs		      =	sr300pc20_effect_negative_regs;
	sensor->rows_num_effect_negative_regs	 = ARRAY_SIZE(sr300pc20_effect_negative_regs);
	
	sensor->effect_sepia_regs			      =	sr300pc20_effect_sepia_regs;
	sensor->rows_num_effect_sepia_regs	  	  = ARRAY_SIZE(sr300pc20_effect_sepia_regs);
	
	sensor->effect_mono_regs			      =	sr300pc20_effect_mono_regs;
	sensor->rows_num_effect_mono_regs	      = ARRAY_SIZE(sr300pc20_effect_mono_regs);

	sensor->effect_aqua_regs				  =	sr300pc20_effect_aqua_regs;
	sensor->rows_num_effect_aqua_regs	  	  = ARRAY_SIZE(sr300pc20_effect_aqua_regs);
	
	sensor->effect_sharpen_regs 		      =	sr300pc20_effect_sharpen_regs;
	sensor->rows_num_effect_sharpen_regs     = ARRAY_SIZE(sr300pc20_effect_sharpen_regs);
	
	sensor->effect_solarization_regs		   = sr300pc20_effect_solarization_regs;
	sensor->rows_num_effect_solarization_regs = ARRAY_SIZE(sr300pc20_effect_solarization_regs);
	
	sensor->effect_black_white_regs 	       =	sr300pc20_effect_black_white_regs;
	sensor->rows_num_effect_black_white_regs  = ARRAY_SIZE(sr300pc20_effect_black_white_regs);
	

	/*wb*/
	sensor->wb_auto_regs				  =	sr300pc20_wb_auto_regs;
	sensor->rows_num_wb_auto_regs	  	  = ARRAY_SIZE(sr300pc20_wb_auto_regs);

	//sensor->wb_sunny_regs				 =	sr300pc20_wb_sunny_regs;
	//sensor->rows_num_wb_sunny_regs	  	 = ARRAY_SIZE(sr300pc20_wb_sunny_regs);
	
	sensor->wb_cloudy_regs				 =	sr300pc20_wb_cloudy_regs;
	sensor->rows_num_wb_cloudy_regs	 = ARRAY_SIZE(sr300pc20_wb_cloudy_regs);
	
	//sensor->wb_tungsten_regs			 =	sr300pc20_wb_tungsten_regs;
	//sensor->rows_num_wb_tungsten_regs	 = ARRAY_SIZE(sr300pc20_wb_tungsten_regs);
	//Changed reg table name to fit UI's name
	sensor->wb_daylight_regs				 =	sr300pc20_wb_daylight_regs;
	sensor->rows_num_wb_daylight_regs	  	 = ARRAY_SIZE(sr300pc20_wb_daylight_regs);
	sensor->wb_incandescent_regs				 =	sr300pc20_wb_incandescent_regs;
	sensor->rows_num_wb_incandescent_regs	  	 = ARRAY_SIZE(sr300pc20_wb_incandescent_regs);

	sensor->wb_fluorescent_regs 		  =	sr300pc20_wb_fluorescent_regs;
	sensor->rows_num_wb_fluorescent_regs  = ARRAY_SIZE(sr300pc20_wb_fluorescent_regs);

	//sensor->wb_cwf_regs 		  =	sr300pc20_wb_cwf_regs;
	//sensor->rows_num_wb_cwf_regs  = ARRAY_SIZE(sr300pc20_wb_cwf_regs);


	/*metering*/
	sensor->metering_matrix_regs		  =	sr300pc20_metering_matrix_regs;
	sensor->rows_num_metering_matrix_regs	  	  = ARRAY_SIZE(sr300pc20_metering_matrix_regs);

	sensor->metering_center_regs		  =	sr300pc20_metering_center_regs;
	sensor->rows_num_metering_center_regs	  	  = ARRAY_SIZE(sr300pc20_metering_center_regs);

	sensor->metering_spot_regs			  =	  sr300pc20_metering_spot_regs;
	sensor->rows_num_metering_spot_regs	  		  = ARRAY_SIZE(sr300pc20_metering_spot_regs);
	
	/*EV*/
	sensor->ev_minus_4_regs 			 =	sr300pc20_ev_minus_4_regs;
	sensor->rows_num_ev_minus_4_regs	 = ARRAY_SIZE(sr300pc20_ev_minus_4_regs);

	sensor->ev_minus_3_regs 			 =	sr300pc20_ev_minus_3_regs;
	sensor->rows_num_ev_minus_3_regs	 = ARRAY_SIZE(sr300pc20_ev_minus_3_regs);

	sensor->ev_minus_2_regs 			 =	sr300pc20_ev_minus_2_regs;
	sensor->rows_num_ev_minus_2_regs	  = ARRAY_SIZE(sr300pc20_ev_minus_2_regs);

	sensor->ev_minus_1_regs 			 =	sr300pc20_ev_minus_1_regs;
	sensor->rows_num_ev_minus_1_regs	 = ARRAY_SIZE(sr300pc20_ev_minus_1_regs);

	sensor->ev_default_regs 			 =	sr300pc20_ev_default_regs;
	sensor->rows_num_ev_default_regs	 = ARRAY_SIZE(sr300pc20_ev_default_regs);

	sensor->ev_plus_1_regs				 =	sr300pc20_ev_plus_1_regs;
	sensor->rows_num_ev_plus_1_regs	 = ARRAY_SIZE(sr300pc20_ev_plus_1_regs);

	sensor->ev_plus_2_regs				 =	sr300pc20_ev_plus_2_regs;
	sensor->rows_num_ev_plus_2_regs	 = ARRAY_SIZE(sr300pc20_ev_plus_2_regs);

	sensor->ev_plus_3_regs				 =	sr300pc20_ev_plus_3_regs;
	sensor->rows_num_ev_plus_3_regs	 = ARRAY_SIZE(sr300pc20_ev_plus_3_regs);

	sensor->ev_plus_4_regs				 =	sr300pc20_ev_plus_4_regs;
	sensor->rows_num_ev_plus_4_regs	 = ARRAY_SIZE(sr300pc20_ev_plus_4_regs);

	
	/*contrast*/
	sensor->contrast_minus_2_regs		 	 =	sr300pc20_contrast_minus_2_regs;
	sensor->rows_num_contrast_minus_2_regs	 = ARRAY_SIZE(sr300pc20_contrast_minus_2_regs);

	sensor->contrast_minus_1_regs		     =	sr300pc20_contrast_minus_1_regs;
	sensor->rows_num_contrast_minus_1_regs	 = ARRAY_SIZE(sr300pc20_contrast_minus_1_regs);
  
	sensor->contrast_default_regs			 =	sr300pc20_contrast_default_regs;
	sensor->rows_num_contrast_default_regs  = ARRAY_SIZE(sr300pc20_contrast_default_regs);

	sensor->contrast_plus_1_regs			 =	sr300pc20_contrast_plus_1_regs;
	sensor->rows_num_contrast_plus_1_regs	 = ARRAY_SIZE(sr300pc20_contrast_plus_1_regs);

	sensor->contrast_plus_2_regs			 =	sr300pc20_contrast_plus_2_regs;
	sensor->rows_num_contrast_plus_2_regs	 = ARRAY_SIZE(sr300pc20_contrast_plus_2_regs);
	
	/*sharpness*/ 
	sensor->sharpness_minus_3_regs		     =	sr300pc20_sharpness_minus_3_regs;
	sensor->rows_num_sharpness_minus_3_regs= ARRAY_SIZE(sr300pc20_sharpness_minus_3_regs);

	sensor->sharpness_minus_2_regs		     =	sr300pc20_sharpness_minus_2_regs;
	sensor->rows_num_sharpness_minus_2_regs= ARRAY_SIZE(sr300pc20_sharpness_minus_2_regs);

	sensor->sharpness_minus_1_regs		 	 =	sr300pc20_sharpness_minus_1_regs;
	sensor->rows_num_sharpness_minus_1_regs = ARRAY_SIZE(sr300pc20_sharpness_minus_1_regs);

	sensor->sharpness_default_regs		 	 =	sr300pc20_sharpness_default_regs;
	sensor->rows_num_sharpness_default_regs  = ARRAY_SIZE(sr300pc20_sharpness_default_regs);

	sensor->sharpness_plus_1_regs		     =	sr300pc20_sharpness_plus_1_regs;
	sensor->rows_num_sharpness_plus_1_regs	 =	ARRAY_SIZE(sr300pc20_sharpness_plus_1_regs);

	sensor->sharpness_plus_2_regs		     =	sr300pc20_sharpness_plus_2_regs;
	sensor->rows_num_sharpness_plus_2_regs	 =	ARRAY_SIZE(sr300pc20_sharpness_plus_2_regs);
    
	sensor->sharpness_plus_3_regs		     =	sr300pc20_sharpness_plus_3_regs;
	sensor->rows_num_sharpness_plus_3_regs	 =	ARRAY_SIZE(sr300pc20_sharpness_plus_3_regs);

		
	/*saturation*/
	sensor->saturation_minus_2_regs 	      =	sr300pc20_saturation_minus_2_regs;
	sensor->rows_num_saturation_minus_2_regs = ARRAY_SIZE(sr300pc20_saturation_minus_2_regs);

	sensor->saturation_minus_1_regs 	 	  =	sr300pc20_saturation_minus_1_regs;
	sensor->rows_num_saturation_minus_1_regs = ARRAY_SIZE(sr300pc20_saturation_minus_1_regs);

	sensor->saturation_default_regs 	      =	sr300pc20_saturation_default_regs;
	sensor->rows_num_saturation_default_regs  = ARRAY_SIZE(sr300pc20_saturation_default_regs);

	sensor->saturation_plus_1_regs		       =	sr300pc20_saturation_plus_1_regs;
	sensor->rows_num_saturation_plus_1_regs	= ARRAY_SIZE(sr300pc20_saturation_plus_1_regs);

	sensor->saturation_plus_2_regs		       =	sr300pc20_saturation_plus_2_regs;
	sensor->rows_num_saturation_plus_2_regs   = ARRAY_SIZE(sr300pc20_saturation_plus_2_regs);

	
	/*zoom*/
	sensor->zoom_00_regs					 =	sr300pc20_zoom_00_regs;
	sensor->rows_num_zoom_00_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_00_regs);

	sensor->zoom_01_regs					 =	sr300pc20_zoom_01_regs;
	sensor->rows_num_zoom_01_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_01_regs);

	sensor->zoom_02_regs					 =	sr300pc20_zoom_02_regs;
	sensor->rows_num_zoom_02_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_02_regs);

	sensor->zoom_03_regs					 =	sr300pc20_zoom_03_regs;
	sensor->rows_num_zoom_03_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_03_regs);

	sensor->zoom_04_regs					 =	sr300pc20_zoom_04_regs;
	sensor->rows_num_zoom_04_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_04_regs);

	sensor->zoom_05_regs					 =	sr300pc20_zoom_05_regs;
	sensor->rows_num_zoom_05_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_05_regs);

	sensor->zoom_06_regs					 =	sr300pc20_zoom_06_regs;
	sensor->rows_num_zoom_06_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_06_regs);

	sensor->zoom_07_regs					 =	sr300pc20_zoom_07_regs;
	sensor->rows_num_zoom_07_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_07_regs);

	sensor->zoom_08_regs					 =	sr300pc20_zoom_08_regs;
	sensor->rows_num_zoom_08_regs	  		  = ARRAY_SIZE(sr300pc20_zoom_08_regs);

	
	/*scene mode*/
	sensor->scene_none_regs 			 		=	sr300pc20_scene_none_regs;
	sensor->rows_num_scene_none_regs	  		 = ARRAY_SIZE(sr300pc20_scene_none_regs);

	sensor->scene_portrait_regs 		 		=	sr300pc20_scene_portrait_regs;
	sensor->rows_num_scene_portrait_regs	  	= ARRAY_SIZE(sr300pc20_scene_portrait_regs);

	sensor->scene_nightshot_regs			   =	sr300pc20_scene_nightshot_regs;
	sensor->rows_num_scene_nightshot_regs	  	 = ARRAY_SIZE(sr300pc20_scene_nightshot_regs);

	sensor->scene_backlight_regs			  =	sr300pc20_scene_backlight_regs;
	sensor->rows_num_scene_backlight_regs	   = ARRAY_SIZE(sr300pc20_scene_backlight_regs);

	sensor->scene_landscape_regs			   =	sr300pc20_scene_landscape_regs;
	sensor->rows_num_scene_landscape_regs	  	 = ARRAY_SIZE(sr300pc20_scene_landscape_regs);

	sensor->scene_sports_regs			      =	sr300pc20_scene_sports_regs;
	sensor->rows_num_scene_sports_regs	  	 = ARRAY_SIZE(sr300pc20_scene_sports_regs);

	sensor->scene_party_indoor_regs 	 	  =	sr300pc20_scene_party_indoor_regs;
	sensor->rows_num_scene_party_indoor_regs  = ARRAY_SIZE(sr300pc20_scene_party_indoor_regs);

	sensor->scene_beach_snow_regs				 =	sr300pc20_scene_beach_snow_regs;
	sensor->rows_num_scene_beach_snow_regs	  	 = ARRAY_SIZE(sr300pc20_scene_beach_snow_regs);

	sensor->scene_sunset_regs			 		 =	sr300pc20_scene_sunset_regs;
	sensor->rows_num_scene_sunset_regs	  		  = ARRAY_SIZE(sr300pc20_scene_sunset_regs);

	sensor->scene_duskdawn_regs 				 =	sr300pc20_scene_duskdawn_regs;
	sensor->rows_num_scene_duskdawn_regs	  	 = ARRAY_SIZE(sr300pc20_scene_duskdawn_regs);

	sensor->scene_fall_color_regs				 =	sr300pc20_scene_fall_color_regs;
	sensor->rows_num_scene_fall_color_regs	  	  = ARRAY_SIZE(sr300pc20_scene_fall_color_regs);

	sensor->scene_fireworks_regs				 =	sr300pc20_scene_fireworks_regs;
	sensor->rows_num_scene_fireworks_regs	  	  = ARRAY_SIZE(sr300pc20_scene_fireworks_regs);
	
	sensor->scene_candle_light_regs 	 		=	sr300pc20_scene_candle_light_regs;
	sensor->rows_num_scene_candle_light_regs	= ARRAY_SIZE(sr300pc20_scene_candle_light_regs);

	sensor->scene_text_regs			   =	sr300pc20_scene_text_regs;
	sensor->rows_num_scene_text_regs	  	 = ARRAY_SIZE(sr300pc20_scene_text_regs);

		
	/*fps*/
	sensor->fps_auto_regs				 =	sr300pc20_fps_auto_regs;
	sensor->rows_num_fps_auto_regs	  		  = ARRAY_SIZE(sr300pc20_fps_auto_regs);

	sensor->fps_5_regs					 =	sr300pc20_fps_5_regs;
	sensor->rows_num_fps_5_regs	  		  = ARRAY_SIZE(sr300pc20_fps_5_regs);

	sensor->fps_7_regs					 =	sr300pc20_fps_7_regs;
	sensor->rows_num_fps_7_regs	  		  = ARRAY_SIZE(sr300pc20_fps_7_regs);

	sensor->fps_10_regs 				 =	sr300pc20_fps_10_regs;
	sensor->rows_num_fps_10_regs	  		  = ARRAY_SIZE(sr300pc20_fps_10_regs);

	sensor->fps_15_regs 				 =	sr300pc20_fps_15_regs;
	sensor->rows_num_fps_15_regs	  		  = ARRAY_SIZE(sr300pc20_fps_15_regs);

	sensor->fps_20_regs 				 =	sr300pc20_fps_20_regs;
	sensor->rows_num_fps_20_regs	  		  = ARRAY_SIZE(sr300pc20_fps_20_regs);

	sensor->fps_25_regs 				 =	sr300pc20_fps_25_regs;
	sensor->rows_num_fps_25_regs	  		  = ARRAY_SIZE(sr300pc20_fps_25_regs);

	sensor->fps_30_regs 				 =	sr300pc20_fps_30_regs;
	sensor->rows_num_fps_30_regs 		  = ARRAY_SIZE(sr300pc20_fps_30_regs);
	
	sensor->fps_60_regs 				 =	sr300pc20_fps_60_regs;
	sensor->rows_num_fps_60_regs	  		  = ARRAY_SIZE(sr300pc20_fps_60_regs);

	sensor->fps_120_regs 				 =	sr300pc20_fps_120_regs;
	sensor->rows_num_fps_120_regs	  		  = ARRAY_SIZE(sr300pc20_fps_120_regs);
	

	
	/*quality*/
	sensor->quality_superfine_regs			 =	sr300pc20_quality_superfine_regs;
	sensor->rows_num_quality_superfine_regs	  = ARRAY_SIZE(sr300pc20_quality_superfine_regs);

	sensor->quality_fine_regs			 =	sr300pc20_quality_fine_regs;
	sensor->rows_num_quality_fine_regs	  = ARRAY_SIZE(sr300pc20_quality_fine_regs);

	sensor->quality_normal_regs 		   =	sr300pc20_quality_normal_regs;
	sensor->rows_num_quality_normal_regs  = ARRAY_SIZE(sr300pc20_effect_normal_regs);

	sensor->quality_economy_regs			 =	sr300pc20_quality_economy_regs;
	sensor->rows_num_quality_economy_regs   = ARRAY_SIZE(sr300pc20_quality_economy_regs);

	
	/*preview size */
	sensor->preview_size_176x144_regs	        =	sr300pc20_preview_size_176x144_regs;
	sensor->rows_num_preview_size_176x144_regs	 = ARRAY_SIZE(sr300pc20_preview_size_176x144_regs);

	sensor->preview_size_320x240_regs	         =	sr300pc20_preview_size_320x240_regs; 
	sensor->rows_num_preview_size_320x240_regs	  = ARRAY_SIZE(sr300pc20_preview_size_320x240_regs);

	sensor->preview_size_352x288_regs	          =	sr300pc20_preview_size_352x288_regs; 
	sensor->rows_num_preview_size_352x288_regs	  = ARRAY_SIZE(sr300pc20_preview_size_352x288_regs);

	sensor->preview_size_640x480_regs	          =	sr300pc20_preview_size_640x480_regs; 
	sensor->rows_num_preview_size_640x480_regs	  = ARRAY_SIZE(sr300pc20_preview_size_640x480_regs);

	sensor->preview_size_704x576_regs	 		=	sr300pc20_preview_size_704x576_regs; 
	sensor->rows_num_preview_size_704x576_regs	 = ARRAY_SIZE(sr300pc20_preview_size_704x576_regs);

	sensor->preview_size_720x480_regs	 		=	sr300pc20_preview_size_720x480_regs; 
	sensor->rows_num_preview_size_720x480_regs	 = ARRAY_SIZE(sr300pc20_preview_size_720x480_regs);
	
	sensor->preview_size_800x480_regs	        =	sr300pc20_preview_size_800x480_regs;
	sensor->rows_num_preview_size_800x480_regs	 = ARRAY_SIZE(sr300pc20_preview_size_800x480_regs);

	sensor->preview_size_800x600_regs	        =	sr300pc20_preview_size_800x600_regs;
	sensor->rows_num_preview_size_800x600_regs	 = ARRAY_SIZE(sr300pc20_preview_size_800x600_regs);

	sensor->preview_size_1024x600_regs	         =	sr300pc20_preview_size_1024x600_regs; 
	sensor->rows_num_preview_size_1024x600_regs	  = ARRAY_SIZE(sr300pc20_preview_size_1024x600_regs);

	sensor->preview_size_1024x768_regs	          =	sr300pc20_preview_size_1024x768_regs; 
	sensor->rows_num_preview_size_1024x768_regs	  = ARRAY_SIZE(sr300pc20_preview_size_1024x768_regs);

	sensor->preview_size_1280x960_regs	          =	sr300pc20_preview_size_1280x960_regs; 
	sensor->rows_num_preview_size_1280x960_regs	  = ARRAY_SIZE(sr300pc20_preview_size_1280x960_regs);

	sensor->preview_size_1600x960_regs	 		=	sr300pc20_preview_size_1600x960_regs; 
	sensor->rows_num_preview_size_1600x960_regs	 = ARRAY_SIZE(sr300pc20_preview_size_1600x960_regs);

	sensor->preview_size_1600x1200_regs	 		=	sr300pc20_preview_size_1600x1200_regs; 
	sensor->rows_num_preview_size_1600x1200_regs	 = ARRAY_SIZE(sr300pc20_preview_size_1600x1200_regs);

	sensor->preview_size_2048x1232_regs	        =	sr300pc20_preview_size_2048x1232_regs;
	sensor->rows_num_preview_size_2048x1232_regs	 = ARRAY_SIZE(sr300pc20_preview_size_2048x1232_regs);

	sensor->preview_size_2048x1536_regs	         =	sr300pc20_preview_size_2048x1536_regs; 
	sensor->rows_num_preview_size_2048x1536_regs	  = ARRAY_SIZE(sr300pc20_preview_size_2048x1536_regs);

	sensor->preview_size_2560x1920_regs	          =	sr300pc20_preview_size_2560x1920_regs; 
	sensor->rows_num_preview_size_2560x1920_regs	  = ARRAY_SIZE(sr300pc20_preview_size_2560x1920_regs);
  
	
	/*Capture size */
	sensor->capture_size_640x480_regs	 		=	sr300pc20_capture_size_640x480_regs;
	sensor->rows_num_capture_size_640x480_regs	 = ARRAY_SIZE(sr300pc20_capture_size_640x480_regs);

	sensor->capture_size_720x480_regs  			=	sr300pc20_capture_size_720x480_regs; 
	sensor->rows_num_capture_size_720x480_regs	 = ARRAY_SIZE(sr300pc20_capture_size_720x480_regs);

	sensor->capture_size_800x480_regs	 		=	sr300pc20_capture_size_800x480_regs;
	sensor->rows_num_capture_size_800x480_regs	 = ARRAY_SIZE(sr300pc20_capture_size_800x480_regs);

	sensor->capture_size_800x486_regs	 		=	sr300pc20_capture_size_800x486_regs;
	sensor->rows_num_capture_size_800x486_regs	 = ARRAY_SIZE(sr300pc20_capture_size_800x486_regs);

	sensor->capture_size_800x600_regs  			=	sr300pc20_capture_size_800x600_regs; 
	sensor->rows_num_capture_size_800x600_regs	 = ARRAY_SIZE(sr300pc20_capture_size_800x600_regs);

    	sensor->capture_size_1024x600_regs	 		=	sr300pc20_capture_size_1024x600_regs;
	sensor->rows_num_capture_size_1024x600_regs	 = ARRAY_SIZE(sr300pc20_capture_size_1024x600_regs);

	sensor->capture_size_1024x768_regs  			=	sr300pc20_capture_size_1024x768_regs; 
	sensor->rows_num_capture_size_1024x768_regs	 = ARRAY_SIZE(sr300pc20_capture_size_1024x768_regs);

	sensor->capture_size_1280x960_regs  			=	sr300pc20_capture_size_1280x960_regs; 
	sensor->rows_num_capture_size_1280x960_regs	 = ARRAY_SIZE(sr300pc20_capture_size_1280x960_regs);

    	sensor->capture_size_1600x960_regs	 		=	sr300pc20_capture_size_1600x960_regs;
	sensor->rows_num_capture_size_1600x960_regs	 = ARRAY_SIZE(sr300pc20_capture_size_1600x960_regs);

	sensor->capture_size_1600x1200_regs  			=	sr300pc20_capture_size_1600x1200_regs; 
	sensor->rows_num_capture_size_1600x1200_regs	 = ARRAY_SIZE(sr300pc20_capture_size_1600x1200_regs);

	sensor->capture_size_2048x1232_regs  			=	sr300pc20_capture_size_2048x1232_regs; 
	sensor->rows_num_capture_size_2048x1232_regs	 = ARRAY_SIZE(sr300pc20_capture_size_2048x1232_regs);

	sensor->capture_size_2048x1536_regs  			=	sr300pc20_capture_size_2048x1536_regs; 
	sensor->rows_num_capture_size_2048x1536_regs	 = ARRAY_SIZE(sr300pc20_capture_size_2048x1536_regs);

	sensor->capture_size_2560x1536_regs  			=	sr300pc20_capture_size_2560x1536_regs; 
	sensor->rows_num_capture_size_2560x1536_regs	 = ARRAY_SIZE(sr300pc20_capture_size_2560x1536_regs);

	sensor->capture_size_2560x1920_regs  			=	sr300pc20_capture_size_2560x1920_regs; 
	sensor->rows_num_capture_size_2560x1920_regs	 = ARRAY_SIZE(sr300pc20_capture_size_2560x1920_regs);

	
	/*pattern*/
	sensor->pattern_on_regs 			  = sr300pc20_pattern_on_regs;
	sensor->rows_num_pattern_on_regs	  = ARRAY_SIZE(sr300pc20_pattern_on_regs);
	
	sensor->pattern_off_regs			  = sr300pc20_pattern_off_regs;
	sensor->rows_num_pattern_off_regs	  = ARRAY_SIZE(sr300pc20_pattern_off_regs);

	/*AE*/
	sensor->ae_lock_regs			  = sr300pc20_ae_lock_regs;
	sensor->rows_num_ae_lock_regs	  = ARRAY_SIZE(sr300pc20_ae_lock_regs);

		
	sensor->ae_unlock_regs			  = sr300pc20_ae_unlock_regs;
	sensor->rows_num_ae_unlock_regs	  = ARRAY_SIZE(sr300pc20_ae_unlock_regs);


	/*AWB*/

	sensor->awb_lock_regs			  = sr300pc20_awb_lock_regs;
	sensor->rows_num_awb_lock_regs	  = ARRAY_SIZE(sr300pc20_awb_lock_regs);

	sensor->awb_unlock_regs			  = sr300pc20_awb_unlock_regs;
	sensor->rows_num_awb_unlock_regs	  = ARRAY_SIZE(sr300pc20_awb_unlock_regs);

	//ISO//
	sensor->iso_auto_regs			  = sr300pc20_iso_auto_regs;
	sensor->rows_num_iso_auto_regs	  = ARRAY_SIZE(sr300pc20_iso_auto_regs);

	sensor->iso_50_regs			  = sr300pc20_iso_50_regs;
	sensor->rows_num_iso_50_regs	  = ARRAY_SIZE(sr300pc20_iso_50_regs);

	sensor->iso_100_regs			  = sr300pc20_iso_100_regs;
	sensor->rows_num_iso_100_regs	  = ARRAY_SIZE(sr300pc20_iso_100_regs);

	sensor->iso_200_regs			  = sr300pc20_iso_200_regs;
	sensor->rows_num_iso_200_regs	  = ARRAY_SIZE(sr300pc20_iso_200_regs);

	sensor->iso_400_regs			  = sr300pc20_iso_400_regs;
	sensor->rows_num_iso_400_regs	  = ARRAY_SIZE(sr300pc20_iso_400_regs);
