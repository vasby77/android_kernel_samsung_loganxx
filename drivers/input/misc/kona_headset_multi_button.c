...
#define HEADPHONE_DETECT_LEVEL_MAX      691
#define HEADPHONE_DETECT_LEVEL_MAX_2_8      890 // ref. 691 @ 2.1 V
...
#ifdef SELECT_MIC_BIAS_BETWEEN_AUXMIC_BIAS_AND_EXT_LDO
...
static int config_adc_for_accessory_detection()
{
...
int detect_hs_type(struct mic_t *mic_dev)
{
	...
	/*
	 * Only if the platform has requested to do so, check for high voltage
	 * headset, otherwise skip that step.
	 */
#if 0
#ifdef SELECT_MIC_BIAS_BETWEEN_AUXMIC_BIAS_AND_EXT_LDO
	if (mic_dev->gpio_ctrl_ldo >= 0)
...
static int __headset_hw_init_micbias_on(struct mic_t *p)
{
	...
	if (!p->headset_pd->ldo_id) {
		...
		pr_info("Setting regulator mode to standby\n");
