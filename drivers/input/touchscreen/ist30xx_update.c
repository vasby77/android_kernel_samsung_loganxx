...
int ist30xx_fw_update(struct i2c_client *client, const u8 *buf, int size, bool mode)
{
	...
	printk("[ TSP ] *** F/W update ***\n");
	printk("[ TSP ] F/W ver: %08x, addr: 0x%04x ~ 0x%04x\n",
	...
	if (mode) {     /* ISP Mode */
			printk("[ TSP ] ist30xx_fw_update ISP mode\n");
...
int ist30xx_auto_fw_update(struct ist30xx_data *data)
{
	...
	bool isp_mode = false;
	printk("[TSP]ist30xx_auto_fw_update\n");
	...
	while (retry--) {
		ret = ist30xx_fw_update(data->client, ist30xx_fw, sizeof(ist30xx_fw),
					isp_mode);
		if (!ret)
			break;
	}
...
int ist30xx_auto_param_update(struct ist30xx_data *data)
{
	...
	mutex_lock(&ist30xx_mutex);
	ret = ist30xx_param_update(data->client, ist30xx_param, sizeof(ist30xx_param));
	mutex_unlock(&ist30xx_mutex);
