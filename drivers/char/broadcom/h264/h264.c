...
static int h264_clk_enabled;
static int h264_clk_need_resume;
#endif
...
static int h264_suspend(struct platform_device *p_dev, pm_message_t state)
{
	printk(KERN_ERR "%s\n",__func__);
	if(h264_clk_enabled == 1) {
		...
		h264_clk_need_resume = 1;
	}
	return 0;
}

static int h264_resume(struct platform_device *p_dev)
{
	int ret;
	
	printk(KERN_ERR "%s\n",__func__);
	
	if(h264_clk_need_resume == 1) {
...
#endif
	usleep_range(50, 100); /* buffer to ensure everything above are done. */
	h264_clk_need_resume = 0;
	}

	return 0;
}

static int __devinit h264_probe(struct platform_device *pdev)
{
	printk(KERN_ERR "%s\n",__func__);
	return 0;
...
int __init h264_init(void)
{
	...
	printk(KERN_ERR "%s: register platform driver\n",__func__);
	ret = platform_driver_register(&gPlatform_driver);
	if (ret < 0) {
