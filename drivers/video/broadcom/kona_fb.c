...
char *get_seq(DISPCTRL_REC_T *rec)
{
	char *buff, *dst;
...
//{{ Mark for GetLog
	...
	if (ret) {
		...
		if (ret==-1) {
			konafb_error("Maybe LCD is NOT connected!\n");
			lcd_exist = 0; //  0 means LCD is not connected.
		} else {
...
#if defined(CONFIG_LCD_NT35510_SUPPORT)
...
void kona_fb_esd_hw_reset(void)
{
	struct kona_fb *fb = g_kona_fb;
	konafb_error("+\n");

	if (fb == NULL) {
		konafb_error("WARNING!!! fb is NULL\n");
		return;
	}
	if (fb->is_display_found == 0) {
		konafb_error("WARNING!!! fb->is_display_found is zero\n");
		return;
	}

	konafb_info("turn off lcd\n");
	mutex_lock(&fb->update_sem);
	fb->g_stop_drawing = 1;
	...
	if (fb->display_ops->power_control(fb->display_hdl,
				       CTRL_SCREEN_OFF))
		konafb_error("Failed to blank this display device!\n");
	disable_display(g_kona_fb);
	kona_clock_stop(g_kona_fb);

	konafb_info("turn on lcd\n");
	if (enable_display(g_kona_fb)) {
		konafb_error("Failed to enable this display device\n");
	}

	/* re-draw exist frame buffer to lcd panel*/
	/* memset(fb->fb.screen_base, 0, framesize); */
	kona_clock_start(g_kona_fb);
	fb->display_ops->update(fb->display_hdl,
			fb->fb.var.yoffset ? fb->buff1 : fb->buff0,
