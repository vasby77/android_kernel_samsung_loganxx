...
static Int32 DSI_DCS_Read(DISPDRV_HANDLE_T drvH, UInt8 reg, UInt8 *rxBuff, UInt8 buffLen);
...
#define CONFIG_BL_SPLASH_SCREEN
...
Int32 DSI_Open(DISPDRV_HANDLE_T drvH)
{
	...
	if( !g_display_enabled ) /* Skip panel reset on first boot */
		hw_reset(drvH, FALSE);
#if 0
...
static Int32 DSI_SetMaxRtnPktSize(DISPDRV_HANDLE_T drvH, UInt8 size)
{
	...
	if (cslRes != CSL_LCD_OK) {
		...
		DSI_ERR("[DISPDRV]:	ERR: Setting Max. Return Packet Size [0x%02X]\n\r",
			DSI_DT_SH_MAX_RET_PKT_SIZE);
		res = -1;
	}
	return res;
...
static Int32 DSI_DCS_Read(DISPDRV_HANDLE_T drvH, UInt8 reg, UInt8 *rxBuff, UInt8 buffLen)
{
	...
	if ((cslRes != CSL_LCD_OK) ||
		...
		DSI_ERR("[DISPDRV]:	ERR: Reading From Reg[0x%08X]\n\r", reg);
		res = -1;
	} else {
	...
	case CTRL_PWR_ON:
		...
		case STATE_PWR_OFF:
			...
			if(!g_display_enabled)
				panel_initialize(info->init_seq);	/* Send panel init sequence */
			else
				g_display_enabled = 0;
	...
	DISPDRV_CB_T apiCb)
{
	...
	if ( pPanel->disp_info->vmode && !strcmp("S6E63M0X3", pPanel->disp_info->name))
		req.lineCount += 2;
#endif
