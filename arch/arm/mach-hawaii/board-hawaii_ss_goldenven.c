...
static struct board_wifi_info brcm_wifi_data = {
	.wl_reset_gpio = 3,
	.host_wake_gpio = 74,
	.board_nvram_file = "/system/vendor/firmware/fw_wifi_nvram_4334.txt",
...
static void tc360_power(bool on)
{
	...
	if (on)
	{
		reg_val = readl(HW_IO_PHYS_TO_VIRT(PAD_CTRL_BASE_ADDR) + 0xA4); // PAD_CTRL_GPIO26. 
		reg_val |= 0x20;  // Add PullUP. 
		reg_val &= ~0x40;  // Remove PullDown. 
		writel(reg_val,HW_IO_PHYS_TO_VIRT(PAD_CTRL_BASE_ADDR) + 0xA4); // PAD_CTRL_GPIO26. 
	...
	{
		reg_val = readl(HW_IO_PHYS_TO_VIRT(PAD_CTRL_BASE_ADDR) + 0xA4); // PAD_CTRL_GPIO26. 
		reg_val &= ~0x20;  // Remove PullUP. 
		reg_val |= 0x40;  // Add PullDown. 
		writel(reg_val,HW_IO_PHYS_TO_VIRT(PAD_CTRL_BASE_ADDR) + 0xA4); // PAD_CTRL_GPIO26. 
...
struct kona_fb_platform_data konafb_devices[] __initdata = {
	{
		...
		.cmnd_LP = true,
		.te_ctrl = false,
		.col_mod_i = 2,  /*DISPDRV_FB_FORMAT_xBGR8888*/
...
static struct platform_device s6e63m0x_panel_device = {
	.name = "S6E63M0",
	.id = -1,
};
#endif
...
#ifdef CONFIG_LCD_S6E63M0X_SUPPORT
	platform_device_register(&s6e63m0x_panel_device);
#endif	
...
MACHINE_START(HAWAII, "hawaii_ss_goldenven")
	.atag_offset = 0x100,
