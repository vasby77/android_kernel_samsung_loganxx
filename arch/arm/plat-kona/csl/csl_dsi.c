...
#if 0
void enable_axipv_clock_debug(struct axipv_config_t *config);
	...
	for (;;) {
		...
		if (osStat != OSSTATUS_SUCCESS) {
			if (osStat == OSSTATUS_TIMEOUT) {
				//enable_axipv_clock_debug(updMsg.dsiH->axipvCfg);
				//dump_regs();
			...
			int tx_done = cslDsiPixTxStop(&updMsg);
			if (tx_done == CSL_LCD_OK) {
				osStat = OSSTATUS_SUCCESS;
				res = CSL_LCD_OK;
				printk("recovered %d\n", __LINE__);
			} else {
				printk("didn't recovered %d\n", __LINE__);
			}
...
static inline int cslDsiCheckCompletion(DSI_HANDLE dsiH)
{
	...
	uint32_t enabled_int = chal_dsi_get_ena_int(dsiH->chalH);
	if (int_status & enabled_int) {
		printk("DSI ISR was pending\n");
		chal_dsi_ena_int(dsiH->chalH, 0);
		chal_dsi_clr_int(dsiH->chalH, int_status);
		ret = 0;
		goto done;
...
static CSL_LCD_RES_T cslDsiWaitForInt(DSI_HANDLE dsiH, UInt32 tout_msec)
{
	...
	osRes =
		OSSEMAPHORE_Obtain(dsiH->semaInt, 0);	
	}
#else
	osRes =
	    OSSEMAPHORE_Obtain(dsiH->semaInt, TICKS_IN_MILLISECONDS(tout_msec));
	...
	if (osRes != OSSTATUS_SUCCESS) {
		...
		int tx_done = cslDsiCheckCompletion(dsiH);
		if (tx_done == 0) {
			printk("DSI recovered\n");
			res = CSL_LCD_OK;
			goto done;
	}
		cslDsiDisInt(dsiH);
...
read_reply:
		...
		if (res != CSL_LCD_OK) {
			...
			while ((res == CSL_LCD_OS_TOUT) && --tries) {
				...
				//ENABLE_AXIPV_CLK(g_axipvCfg);
				//dump_regs();
				//DISABLE_AXIPV_CLK(g_axipvCfg);
				if (!clientH->hasLock)
					cslDsiEnaIntEvent(dsiH, event);
				chal_dsi_tx_start(dsiH->chalH, TX_PKT_ENG_1,
						FALSE);
...
CSL_LCD_RES_T CSL_DSI_Suspend(CSL_LCD_HANDLE vcH)
{
	...
	if (osStat == OSSTATUS_TIMEOUT) {
		int tx_done = check_pv_state(PV_STOP_EOF, pvCfg);
		if (tx_done < 0) {
	...
	if (req->cslLcdCb == NULL) {
		if (!clientH->hasLock) {
			...
			if (osStat != OSSTATUS_SUCCESS) {
				...
				int tx_done = cslDsiPixTxStop(&updMsgCm);
				if (tx_done == CSL_LCD_OK) {
					osStat = OSSTATUS_SUCCESS;
					res = CSL_LCD_OK;
				}
